...
*   **Control Flow (If/Elif & Match-Case):**
    ...
    *   **Match-Case:** When repeatedly checking the exact same variable or expression against different values, use `match`/`case` structural pattern matching instead of a chain of `if`/`elif` statements, unless doing so would decrease performance.
